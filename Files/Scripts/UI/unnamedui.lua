...
-- KEYBINDS | keybind's callback has 1 argument, it indicates wether the key was pressed down or was released. If it was pressed down the argument is true, if not then false
-- Modified -> Fri Jul 5th 2024
-- Modified -> Dec 5 2024 [ 10:25 AM ] : Fixed Keybind Input Keys. 
-- Modified -> Dec 7 2024 [ 5:45 PM ] : Added UI Toggle, Library:Window({ Text = "Example", Toggle = Enum.KeyCode.Alt })
...
end); task.spawn(function() while task.wait() and iscount() do end end);
...
if not isfolder("Unnamed") then
    ...
    local Chevron =
        request(
        {Url = "https://raw.githubusercontent.com/Rain-Design/Unnamed/main/Icons/Chevron.png", Method = "GET"}
    )
    writefile("Unnamed/Chevron.png", Chevron.Body)

    local Circle =
        request(
        {Url = "https://raw.githubusercontent.com/Rain-Design/Unnamed/main/Icons/Circle.png", Method = "GET"}
    )
...
function library:Window(Info)
    ...
    addCache(UserInputService.InputBegan:Connect(function(key, ischat)
        if ischat then return; end

        -- Parse key
        local presskn = Info.Toggle and tostring(Info.Toggle):split(".")[3]
        local name = tostring(key.KeyCode):split(".")[3] or "unknown"
        if name:lower() == "unknown" then
            name = tostring(key.UserInputType):split(".")[3] or name
        end

        -- UI Toggle
        if presskn == name then
    ...
    function window:Tab(Info)
        ...
        function tabtable:Section(Info)
            ...
            function sectiontable:Keybind(Info)
                ...
                addCache(
                    UserInputService.InputBegan:Connect(
                        function(Key, gameProcessed)
                            -- Parse key
                            local presskn = PressKey and tostring(PressKey):split(".")[3]
                            local name = tostring(Key.KeyCode):split(".")[3] or "unknown"
                            if name:lower() == "unknown" then
                                name = tostring(Key.UserInputType):split(".")[3] or name
                            end
                            ...
                            -- Handle
                            if name == presskn and not (Changing or gameProcessed) then
                            ...
                            MouseMove =
                                ...
                                Mouse.Move:Connect(
                                    function()
                                        ...
                                        local Value =
                                            math.floor(Info.Minimum + ((Info.Maximum - Info.Minimum) * Px))
                                        SizeFromScale = SizeFromScale - (SizeFromScale % 2)
                                        TweenService:Create(
                                            sliderInner,
            ...
            function sectiontable:Input(Info)
                ...
                if
                    (Info.NotClearOnFocus or Info.DontClearOnFocus or Info.DontClearTextOnFocus or
                        Info.NotClearTextOnFocus)
                 then
                    pcall(
                        function()
                ...
                return {
                    SetText = function(txt)
                        textBox.Text = type(txt) == "string" and txt or ""
                        if Info.Flag ~= nil then
                            library.Flags[Info.Flag] = textBox.Text
                        end
                    end,
            ...
            function sectiontable:Dropdown(Info)
                        ...
                        dropdownElementButton.MouseButton1Click:Connect(
                            function()
                                ...
                                TweenService:Create(
                                    ...
                                    TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out),
                                    {
                                        Size = DropdownOpened and
                                            UDim2.new(0, 175, 0, section.Size.Y.Offset + (DropdownY)) or
                                ...
                                TweenService:Create(
                                    ...
                                    TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out),
                                    {
                                        Size = DropdownOpened and UDim2.new(0, 171, 0, DropdownY) or
                                            UDim2.new(0, 171, 0, 0)
                                    }
                                ):Play()
                                TweenService:Create(
                ...
                function insidedropdown:Remove(opt)
                    for _, v in pairs(dropdownContainer:GetChildren()) do
                        if
                            v.ClassName == "Frame" and v:FindFirstChild("dropdownElementText") and
                                v:FindFirstChild("dropdownElementText").Text == opt
                         then
                            DropdownY = DropdownY - 24
                            if DropdownOpened then
                                TweenService:Create(
            ...
            function sectiontable:Button(Info)
                ...
                return {SetText = function(text)
                    buttonText.Text = text or buttonText.Text
                end}
            end
