...
--[[ UI Library: Usage Example By Linen#3485
...
addCache(UserInputService.InputBegan:Connect(function(Input, GameProcessed)
...
local scrollSize
addCache(consoleContainer.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
...
addCache(bottomText.MouseEnter:Connect(function()
Utilities:Tween(bottomText, .125, {TextColor3 = Colors.Accent})
end))

addCache(bottomText.MouseLeave:Connect(function()
Utilities:Tween(bottomText, .125, {TextColor3 = Colors.PrimaryText})
end))

addCache(bottomText.CloseConsole.MouseButton1Click:Connect(function()
...
local function dragify(Frame)
  ...
  addCache(Frame.InputBegan:Connect(function(input)
  ...
  addCache(Frame.InputChanged:Connect(function(input)
  if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
  dragInput = input
  end
  end))
  
  addCache(UserInputService.InputChanged:Connect(function(input)
...
addCache(ResizeButton.MouseButton1Down:Connect(function()
  local ResizeMove, ResizeKill
  
  Utilities:Tween(Window.Main.Bottom.ResizeIcon, .125, {ImageColor3 = Colors.Accent})
  
  ResizeMove = addCache(Mouse.Move:Connect(function()
  Resize()
  end))
  
  ResizeKill = addCache(UserInputService.InputEnded:Connect(function(UserInput)
...
addCache(TabContainer.ChildAdded:Connect(function()
...
addCache(Tab.MouseEnter:Connect(function()
...
addCache(Tab.MouseLeave:Connect(function()
...
addCache(Tab.TabButton.MouseButton1Click:Connect(function()
TabTable:Select()
end))
addCache(Tab.MouseLeave:Connect(function()
if SelectedTab == nil or Tab ~= SelectedTab then
...
addCache(SizeX:GetPropertyChangedSignal("Value"):Connect(function()
...
addCache(SectionContainer.ChildAdded:Connect(function()
...
addCache(CheckButton.MouseEnter:Connect(function()
if not State then
...
addCache(CheckButton.MouseLeave:Connect(function()
if not State then
...
addCache(CheckButton.MouseButton1Click:Connect(function()
State = not State
...
addCache(Button.ButtonFrame.MouseEnter:Connect(function()
Hovering = true
...
addCache(Button.ButtonFrame.MouseLeave:Connect(function()
Hovering = false
...
addCache(Button.ButtonFrame.ButtonButton.MouseButton1Down:Connect(function()
...
addCache(Button.ButtonFrame.ButtonButton.MouseButton1Up:Connect(function()
...
addCache(Button.ButtonFrame.ButtonButton.MouseButton1Click:Connect(function()
...
addCache(Slider.SliderOuter.MouseEnter:Connect(function()
...
addCache(Slider.SliderOuter.MouseLeave:Connect(function()
...
addCache(Slider.SliderOuter.SliderButton.MouseButton1Down:Connect(function()
Utilities:Tween(Slider.SliderOuter.SliderInner, .125, {BackgroundColor3 = Colors.Accent})
end))

addCache(Slider.SliderOuter.SliderButton.MouseButton1Up:Connect(function()
Utilities:Tween(Slider.SliderOuter.SliderInner, .125, {BackgroundColor3 = Colors.DarkerAccent})
end))
...
MouseMove = addCache(Mouse.Move:Connect(function()
...
MouseKill = addCache(UserInputService.InputEnded:Connect(function(UserInput)
if UserInput.UserInputType == Enum.UserInputType.MouseButton1 then
...
function SectionTable:Textbox(Info)
  Info.Text = Info.Text or "Label"
  Info.Color = Info.Color or Colors.PrimaryText
  Info.Callback = Info.Callback or function(...)return(...)end
  Info.TextXAlignment = typeof(Info.TextXAlignment)=="Enum" and Info.TextXAlignment or Enum.TextXAlignment.Center
  Info.TextYAlignment = typeof(Info.TextYAlignment)=="Enum" and Info.TextYAlignment or Enum.TextYAlignment.Center
  ...
  local TxtBox = Label.LabelText
  local enterDown = false 

  addCache(UserInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.Return then 
      enterDown = true 
    end
  end))
  
  addCache(UserInputService.InputEnded:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.Return then 
      enterDown = false 
    end
  end))
  
  addCache(TxtBox.FocusLost:Connect(function()
    --giving some time to InputBegan/InputEnded to fire
    for i = 1, 5 do
      task.wait()
    end
...
addCache(Dropdown.DropdownFrame.MouseEnter:Connect(function()
if not State then
Utilities:Tween(Dropdown.DropdownFrame, .125, {BackgroundColor3 = Colors.Hovering})
end
end))

addCache(Dropdown.DropdownFrame.MouseLeave:Connect(function()
if not State then
...
if Info.ChangeText then
Dropdown.DropdownFrame.DropdownText.Text = (Info.SaveText and ("%s: %s"):format(Info.Text, v) or v)
end
end
...
function DropdownTable:Get(str)
  str = type(str)=="string" and str or nil;if not str then return false; end
  for i,v in next, DropdownContainer:GetChildren() do
    local element = v.Name=="DropdownElement" and v:FindFirstChild("DropdownElementText") and v or nil
...
addCache(DropdownElement.MouseEnter:Connect(function()
...
addCache(DropdownElement.MouseLeave:Connect(function()
if not table.find(MultiTable, DropdownElement.DropdownElementText.Text) then
...
addCache(DropdownElement.DropdownElementButton.MouseButton1Click:Connect(function()
...
addCache(Dropdown.DropdownFrame.DropdownButton.MouseButton1Click:Connect(function()
State = not State
